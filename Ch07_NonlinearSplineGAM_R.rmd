...
Your code:
```{r,echo=TRUE}
Train_College <- slice_sample(College,n = 222)
Test_College <- anti_join(College,Train_College)

Mod_College <- regsubsets(Outstate ~.,Train_College)

Mod_summary <- summary(Mod_College)
names(Mod_summary)
Mod_summary$rsq

plot(Mod_College, scale = "r2")
plot(Mod_College, scale = "adjr2")
plot(Mod_College, scale = "Cp")
plot(Mod_College, scale = "bic")

```

Your answer:The model uses Private,Accept,F.Undergrad,Room.Board,Personal,PhD,Perc.alumni,expend.
...
```{r,echo=TRUE}
library(gam)
Gam_College <- gam(Outstate ~ Private + s(Accept) + s(F.Undergrad) + s(Room.Board) + s(Personal) + s(PhD) + s(perc.alumni) + s(Expend),data = Train_College)

summary(Gam_College)

```

Your answer:The only two predictors that don't appear to have a significant relationship are Personal, and PhD. Accept, F. Undergrad, Perc alumni, and expenditure have the most complex nonlinear relationship with Outstate.
...
Your code:
```{r,echo=TRUE}
Gam_Predict <- predict(Gam_College, newdata = Test_College)
Gam_Predict <- as.data.frame(Gam_Predict)
...
Your answer: Acceptance, Room/Board, PhD, Expenditure, F undergrad, and Perc alumni. 
